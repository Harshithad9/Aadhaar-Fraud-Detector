...
// const API_BASE_URL = "http://localhost:5000"; // Only for local development

// Point to your local backend server running on port 8080
...
document.addEventListener("DOMContentLoaded", () => {
      ...
      if (serviceType === "single" || serviceType === "batch") {
        ...
        if (uploadDescription)
          uploadDescription.textContent =
            "Contact us to get API access for integrating Aadhaar verification into your applications."
  ...
  if (singleBtn && batchBtn) {
    console.log("[v0] Initializing verification page...")
  ...
  if (verifyForm) {
    ...
    console.log("[v0] Form submit listener added")
...
function setMode(mode) {
  ...
  console.log(`[v0] Setting mode to: ${mode}`)
  ...
  if (uploadTitle && uploadDescription) {
    if (mode === "single") {
      ...
      uploadDescription.textContent =
        "Upload Aadhaar card images for verification. Our system automatically detects if the uploaded image is actually an Aadhaar card."
    } else {
      uploadTitle.textContent = "Batch Aadhaar Verification"
      uploadDescription.textContent =
        "Upload a ZIP file containing multiple Aadhaar card images for batch processing. Our system will verify each image individually."
    }
  }
...
function showLoading(show) {
  ...
  if (loadingSpinner) {
    ...
    console.log(`[v0] Loading spinner: ${show ? "shown" : "hidden"}`)
  }
}

async function handleVerificationSubmit(e) {
  e.preventDefault()
  console.log("[v0] Form submitted, mode:", currentMode)
  ...
  if (currentMode === "single") {
    ...
    console.log("[v0] Front file:", frontFile.name, "size:", frontFile.size)

    const backFile = document.getElementById("back").files[0]
    if (backFile) {
      formData.append("back", backFile)
      console.log("[v0] Back file:", backFile.name, "size:", backFile.size)
    }
  } else {
    ...
    console.log("[v0] ZIP file:", zipFile.name, "size:", zipFile.size)
  }
  ...
  if (currentMode === "single") {
    ...
    console.log("[v0] QR check enabled:", qrCheckbox?.checked)
  }

  const endpoint = currentMode === "single" ? `${API_BASE_URL}/api/verify_single` : `${API_BASE_URL}/api/verify_zip`
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        try {
          ...
          console.warn("[v0] Could not parse error body:", inner)
        }
      }
    ...
    console.log("[v0] API Response received successfully:", data)
    showLoading(false)
    ...
    if (data.success) {
      ...
      if (data.error === "NOT_AADHAAR") {
        ...
        if (verificationResults) {
          verificationResults.innerHTML = `<p style="color: #ef4444;">API Error: ${data.error || "Unknown error"}</p>`
        }
      }
    }
  } catch (error) {
    ...
    showLoading(false)
    if (verificationResults) {
      verificationResults.innerHTML = `<p style="color: #ef4444;">Error: ${error.message}</p>`
    }
  }
}
...
function displaySingleResult(result) {
  ...
  console.log("[v0] Displaying single result:", result)
...
function showSingleFullDetails(result) {
  ...
  if (result.extracted) {
    resultHTML += `
            <h5 style="margin-top: 20px; margin-bottom: 10px; color: #1a3a34;">Extracted Information:</h5>
  ...
  if (result.indicators && result.indicators.length > 0) {
    resultHTML += `
            <h5 style="margin-top: 20px; margin-bottom: 10px; color: #1a3a34;">Verification Indicators:</h5>
            <div class="verification-details">
                ${result.indicators
                  .map(
                    (indicator) => `
  ...
  if (result.annotated_b64) {
    resultHTML += `
            <h5 style="margin-top: 20px; margin-bottom: 10px; color: #1a3a34;">Processed Image:</h5>
            <div style="text-align: center; margin: 20px 0;">
                <img src="data:image/jpeg;base64,${result.annotated_b64}" 
                     style="max-width: 100%; max-height: 500px; border: 1px solid #d1fae5; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);" 
                     alt="Processed Aadhaar image with annotations" />
                <p style="margin-top: 10px; color: #4a6b64; font-size: 14px;">Annotated image showing detected elements and verification markers</p>
            </div>
        `
  }
...
function showBatchFullDetails(results) {
  ...
  if (results.length > 0) {
    resultHTML += `
            <h5 style="margin-bottom: 15px; color: #1a3a34;">Detailed Results:</h5>
...
function generateBatchSummary(results) {
  ...
  const errors = results.filter((r) => r.error && r.error !== "NOT_AADHAAR")
...
function convertSingleToCSV(data) {
  const headers = ["Field", "Value", "Status", "Risk Level", "Details"]
  ...
  const rows = [
    ...
    [
      "Aadhaar Number",
      data.extracted_data.aadhaar || "N/A",
    ...
    [
      "Gender",
      data.extracted_data.gender || "N/A",
    ...
    // Risk breakdown
    [
      "Text Extraction Risk",
      data.risk_breakdown.text_extraction_risk,
      "N/A",
      data.risk_breakdown.text_extraction_risk,
      "",
    ],
    [
      ...
      data.risk_breakdown.data_consistency_risk,
      "N/A",
      data.risk_breakdown.data_consistency_risk,
      "",
    ],
